...
知乎有权但无义务对用户发布的内容进行审核，有权根据相关证据结合法律法规及知乎社区指导原则对侵权信息进行处理。
个人隐私
...
如果用户不希望被搜索引擎检索，可在「个人设置」中修改「个人信息」部分的设置，对于站外的用户与搜索引擎，你的姓名会显示为「知乎用户」，头像图片也将被隐藏。
您使用或继续使用我们的服务，即意味着同意我们按照知乎《知乎隐私保护指引》收集、使用、储存和分享您的相关信息。详情请参见《知乎隐私保护指引》
知乎币
知乎币是专供知乎用户在知乎平台使用的虚拟币，知乎币可用于知乎平台上赞赏、购买和消费内容产品。除此之外，不得用于其他用途。
如果用户使用他人代充或以其他违规方式充值造成其他用户或第三方权益受损时，不得因此要求知乎作任何补偿或赔偿，知乎保留随时冻结其帐户余额、暂停或终止其使用各项充值服务及禁用其帐户的权利。如知乎有理由相信用户的知乎币帐户或使用情况涉及违规充值、作弊或异常状况，知乎有权拒绝该用户继续使用知乎币，并按本协议采取冻结、禁用帐户等相关措施。
知乎币和人民币的兑换比例以用户购买知乎币的服务页面显示为准。知乎有权根据运营情况随时变更兑换比例，并在用户购买知乎币的服务页面显示。
知乎币一经充值成功，除法律法规明确规定外，在任何情况下不能兑换为法定货币，不能转让他人。用户间交易知乎币构成对本协议的违反，知乎有权不通知用户而采取适当措施，以确保知乎不为违规用户提供知乎币交易的平台服务。知乎币不支持提现功能。
如果用户使用 iOS 设备充值知乎币，必须通过 Apple 应用内购买机制（In-App Purchase）进行支付。为保障帐号安全，请在个人 iOS 设备上使用个人常用 Apple ID 进行操作。
iOS 和其他设备之间的知乎币是独立计算不互通的。若使用同一帐号分别在 iOS 和其他设备上登录知乎时，知乎币不能实现互通共用。使用 iOS 设备时购买的知乎币，只能在 iOS 设备上使用，当使用其他设备登录同一帐号时，不能使用。
侵权举报
处理原则
...
用户在知乎发表的内容仅表明其个人的立场和观点，并不代表知乎的立场或观点。作为内容的发表者，需自行对所发表内容负责，因所发表内容引发的一切纠纷，由该内容的发表者承担全部法律及连带责任。知乎不承担任何法律及连带责任。
知乎不保证网络服务一定能满足用户的要求，也不保证网络服务不会中断，对网络服务的及时性、安全性、准确性也都不作保证。
对于因不可抗力或知乎不能控制的原因造成的网络服务中断或其它缺陷，知乎不承担任何责任，但将尽力减少因此而给用户造成的损失和影响。
协议修改
根据互联网的发展和有关法律、法规及规范性文件的变化，或者因业务发展需要，知乎有权对本协议的条款作出修改或变更，一旦本协议的内容发生变动，知乎将会直接在知乎网站上公布修改之后的协议内容，该公布行为视为知乎已经通知用户修改内容。知乎也可采用电子邮件或私信的传送方式，提示用户协议条款的修改、服务变更、或其它重要事项。
...
<style scoped>
@import "~@/pages/submitPage/RichText.css";
.policy-desc {
  padding: 10px;
  height: 1670px;
  overflow: scroll;
